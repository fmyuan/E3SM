...
#include "shr_assert.h"
  ...
  ! ELM types/variables are replaced by elm_interface_data
...
!-----------------------------------------------------------------------
  ...
  !EOP
  !-----------------------------------------------------------------------
    namelist / elm_pflotran_inparm / pflotran_inputdir, pflotran_prefix
...
!
  ...
  subroutine interface_init(bounds)
    ...
    ! passing ELM total running time (in seconds) to interface_data so that PF can setup its 'final_time'
    call calc_nestep()  ! assign value to nestep
    total_elmstep = nestep - pf_elmnstep0 + 1
    elm_pf_idata%final_time = total_elmstep * dtime
    ...
    elm_pf_idata%elm_lx(1:elm_pf_idata%npx) = &
       (nx-mod(nx,elm_pf_idata%npx))/elm_pf_idata%npx
    do i=1, mod(nx,elm_pf_idata%npx)
       elm_pf_idata%elm_lx(i) = elm_pf_idata%elm_lx(i)+1
    end do
    elm_pf_idata%elm_ly(1:elm_pf_idata%npy) = &
       (ny-mod(ny,elm_pf_idata%npy))/elm_pf_idata%npy
    ...
    ! For ELM/PF: ground surface NOT defined (currently),
    ...
    ! (6) Allocate vectors for data transfer between ELM and PFLOTRAN, after knowing data sizes.
    ...
    call ELMPFLOTRANIDataCreateVec(mpicom)
    ...
    ! coupled module controls betweeen PFLOTRAN and ELM
    ...
    HEAD_BASED = .false.
    elm_pf_idata%head_based = HEAD_BASED
  ...
  !-----------------------------------------------------------------------
    ...
    !if (.not. isinitpf) then    ! always initialize from ELM to PF, if comment out this 'if'block
       ...
       if (.not.pf_tmode .or. .not.pf_hmode) then
          call pflotranModelUpdateTHfromELM(pflotran_m, pf_hmode, pf_tmode)
       endif
       ...
       if (pf_hmode .or. pf_tmode) then
          call pflotranModelSetInternalTHStatesfromELM(pflotran_m, HEAD_BASED)
       endif
    ...
    if (pf_hmode) then      !if coupled 'H' mode between ELM and PFLOTRAN
  ...
  subroutine get_elm_soil_properties(elm_interface_data, bounds, filters)
    ...
    associate( &
         cgridcell                => col_pp%gridcell                            , & !  [integer (:)]  gridcell index of column
         !
         dz                       => elm_interface_data%dz                      , & ! layer thickness depth (m)
         z                        => elm_interface_data%z                       , & ! layer depth (m)
         zi                       => elm_interface_data%zi                      , & ! layer depth (m)
         bd                       => elm_interface_data%bd                      , & !
         bsw                      => elm_interface_data%bsw                     , & !  [real(r8) (:,:)]  Clapp and Hornberger "b" (nlevgrnd)
         hksat                    => elm_interface_data%hksat                   , & !  [real(r8) (:,:)]  hydraulic conductivity at saturation (mm H2O /s) (nlevgrnd)
         sucsat                   => elm_interface_data%sucsat                  , & !  [real(r8) (:,:)]  minimum soil suction (mm) (nlevgrnd)
         watsat                   => elm_interface_data%watsat                  , & !  [real(r8) (:,:)]  volumetric soil water at saturation (porosity) (nlevgrnd)
         watfc                    => elm_interface_data%watfc                   , & !  [real(r8) (:,:)]  volumetric soil water at field capacity (nlevgrnd)
         !
         tkwet                    => elm_interface_data%tkwet                   , & !  [real(r8) (:,:)]  (nlevgrnd)
         tkdry                    => elm_interface_data%tkdry                   , & !  [real(r8) (:,:)]  (nlevgrnd)
         tkfrz                    => elm_interface_data%tkfrz                   , & !  [real(r8) (:,:)]  (nlevgrnd)
         csol                     => elm_interface_data%csol                    , & !  [real(r8) (:,:)]  (nlevgrnd)
         !
         rf_decomp_cascade        => elm_interface_data%bgc%rf_decomp_cascade       , &
         pathfrac_decomp_cascade  => elm_interface_data%bgc%pathfrac_decomp_cascade , &
...
!-------------------------------------------------------------------------------------
    if(pflotran_m%option%ntrandof > 0) then
      ...
      if (spinup_state == 1) then
         ...
         elm_pf_idata%adfactor_ck_c = 1.0_r8
      endif
...
# endif
        ...
        do j = 1, elm_pf_idata%nzelm_mapped
          ...
          if (j <= nlevgrnd) then
            ...
            tkwet_elm_loc(cellcount )  = tkwet(c,j)
            tkdry_elm_loc(cellcount )  = tkdry(c,j)
            tkfrz_elm_loc(cellcount )  = tkfrz(c,j)
            hcvsol_elm_loc(cellcount ) = csol(c,j)

            hksat_x_elm_loc(cellcount ) = hksat(c,j)
            hksat_y_elm_loc(cellcount ) = hksat(c,j)
            hksat_z_elm_loc(cellcount ) = hksat(c,j)

            sucsat_elm_loc( cellcount ) = sucsat(c,j)
            watsat_elm_loc( cellcount ) = watsat(c,j)
            bsw_elm_loc(    cellcount ) = bsw(c,j)
            watfc_elm_loc(  cellcount ) = watfc(c,j)
            bulkdensity_dry_elm_loc( cellcount ) = bd(c,j)
  ...
  !
  ! !DESCRIPTION:
  !  update soil temperature/water content from ELM to PFLOTRAN for driving PF's BGC
  ...
  ! !LOCAL VARIABLES:
    ...
    PetscScalar, pointer :: soillsat_elmp_loc(:), soilisat_elmp_loc(:)   ! 0 -1 of porosity
    PetscScalar, pointer :: soilliq_elmp_loc(:), soilice_elmp_loc(:)     ! kg/m^3 bulk soil
  ...
  !-----------------------------------------------------------------------
    associate ( &
      ...
      !
      dz              => elm_interface_data%dz          , & ! layer thickness depth (m)
      sucsat          => elm_interface_data%sucsat      , & ! minimum soil suction (mm) (nlevgrnd)
      bsw             => elm_interface_data%bsw         , & ! Clapp and Hornberger "b"
      watsat          => elm_interface_data%watsat      , & ! volumetric soil water at saturation (porosity) (nlevgrnd)
      eff_porosity    => elm_interface_data%eff_porosity, & ! porosity - ice-vwc
      watmin          => elm_interface_data%watmin      , & ! col minimum volumetric soil water (nlevsoi)
      sucmin          => elm_interface_data%sucmin      , & ! col minimum allowable soil liquid suction pressure (mm) [Note: sucmin is a negative value, while sucsat is a positive quantity]
      !
      soilpsi         => elm_interface_data%th%soilpsi   ,  & ! soil water matric potential in each soil layer (Pa)
      h2osoi_liq      => elm_interface_data%th%h2osoi_liq,  & ! liquid water (kg/m2)
      h2osoi_ice      => elm_interface_data%th%h2osoi_ice,  & ! ice lens (kg/m2)
      h2osoi_vol      => elm_interface_data%th%h2osoi_vol,  & ! volumetric soil water (0<=h2osoi_vol<=watsat) [m3/m3]
      t_soisno        => elm_interface_data%th%t_soisno  ,  & ! snow-soil temperature (Kelvin)
      !
      t_scalar        => elm_interface_data%bgc%t_scalar ,  & ! soil temperature scalar for decomp
      w_scalar        => elm_interface_data%bgc%w_scalar ,  & ! soil water scalar for decomp
      o_scalar        => elm_interface_data%bgc%o_scalar    & ! fraction by which decomposition is limited by anoxia
    )
    ...
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    call VecGetArrayF90(elm_pf_idata%soilliq_elmp, soilliq_elmp_loc, ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    call VecGetArrayF90(elm_pf_idata%soilice_elmp, soilice_elmp_loc, ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
      ...
      do j = 1, elm_pf_idata%nzelm_mapped
        ...
        if (j<=nlevgrnd) then
          ...
          if (initpfhmode) then
             ...
             soilisat_elmp_loc(cellcount) = isattmp
             soilice_elmp_loc(cellcount)  = h2osoi_ice(c,j) / dz(c,j)           ! kg/m^3
             ...
             if(sattmp<0.0001_r8) then ! h2osoi_liq from 'elm hydrology' could be negative (SEE 'SoilWaerMovementMod.F90': L.841)
                sattmp = 0.0001_r8
             ...
             ! this value may be different from what ELM used, for examples:
             ! (1) if not ice-content adjusted, then in PF,
             !     the PSI is very small (negative) which implies possible water movement
             ! (2) 'soilpsi' primarily used in 'ELM-CN', if only in physical-mode for ELM, it actually didn't have a value
             ...
             if (shr_infnan_isnan(soilpsi(c,j)) .or. &
                ...
                ! when initialization, ELM did NOT assign a value to soilpsi
                soilpsi(c,j) = psitmp                                        ! -Pa
             endif

             soillsat_elmp_loc(cellcount)  = sattmp
             soilliq_elmp_loc(cellcount)   = h2osoi_liq(c,j) / dz(c,j)           ! kg/m^3
    ...
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    call VecRestoreArrayF90(elm_pf_idata%soilliq_elmp, soilliq_elmp_loc, ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    call VecRestoreArrayF90(elm_pf_idata%soilice_elmp, soilice_elmp_loc, ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
...
!-----------------------------------------------------------------------------
  ...
  ! !LOCAL VARIABLES:
    ...
    ! for PF --> ELM (seq.)
    PetscScalar, pointer :: press_elms_loc(:)       ! Pa
    PetscScalar, pointer :: soilliq_elms_loc(:)     ! kgH2O/m3
    PetscScalar, pointer :: soilice_elms_loc(:)     !
    PetscScalar, pointer :: effporosity_elms_loc(:) ! 0 - 1
    PetscScalar, pointer :: sr_pcwmax_elms_loc(:)   ! 0 - 1

    ! for ELM (mpi) --> PF
    PetscScalar, pointer :: qflw_elmp_loc(:)         !   source/sink term for plant Transpiration: unit in mass rate (kgH2O/m3/sec)
    ...
    PetscScalar, pointer :: qfluxw_top_elmp_loc(:)         !   BC in neumann flux type: unit in kgH20/m2/sec (liq water)
    PetscScalar, pointer :: qfluxev_top_elmp_loc(:)        !   BC in neumann flux type: unit in kgH20/m2/sec (evaporation)
    PetscScalar, pointer :: qfluxw_base_elmp_loc(:)        !   BC in neumann flux type: unit in kgH20/m2/sec (liq water)
  ...
  !-----------------------------------------------------------------------
    ...
    dz                => elm_interface_data%dz                            , & ! layer thickness depth (m)
    z                 => elm_interface_data%z                             , & ! layer depth (m)
    snl               => elm_interface_data%snl                           , & ! number of snow layers (negative)
    !
    bsw               => elm_interface_data%bsw                           , &! Clapp and Hornberger "b" (nlevgrnd)
    hksat             => elm_interface_data%hksat                         , &! hydraulic conductivity at saturation (mm H2O /s) (nlevgrnd)
    watsat            => elm_interface_data%watsat                        , &! volumetric soil water at saturation (porosity) (nlevgrnd)
    sucsat            => elm_interface_data%sucsat                        , &! minimum soil suction (mm) (nlevgrnd)
    watmin            => elm_interface_data%watmin                        , &! restriction for min of volumetric soil water, or, residual vwc (-) (nlevgrnd)
    sucmin            => elm_interface_data%sucmin                        , &! restriction for min of soil potential (mm) (nlevgrnd)
    !
    frac_sno_eff      => elm_interface_data%th%frac_sno_eff               , &! [real(r8) (:) ]   fraction of ground covered by snow (0 to 1)
    frac_h2osfc       => elm_interface_data%th%frac_h2osfc                , &! [real(r8) (:) ]   fraction of ground covered by surface water (0 to 1)
    t_soisno          => elm_interface_data%th%t_soisno                   , &! [real(r8) (:,:) ] snow-soil layered temperature [K]
    t_grnd            => elm_interface_data%th%t_grnd                     , &! [real(r8) (:) ]   col ground(-air interface averaged) temperature (Kelvin)
    t_nearsurf        => elm_interface_data%th%t_nearsurf                 , &! [real(r8) (:) ]   col mixed air/veg. temperature near surface (for coupling with PFLOTRAN as BC)
    qflx_top_soil     => elm_interface_data%th%qflx_top_soil              , &! [real(r8) (:) ]   column-level net liq. water input into soil from top (mm/s)
    qflx_evap_h2osfc  => elm_interface_data%th%qflx_evap_h2osfc           , &! [real(r8) (:) ]   column-level p-aggregated evaporation flux from h2osfc (mm H2O/s) [+ to atm]
    qflx_evap_soil    => elm_interface_data%th%qflx_evap_soil             , &! [real(r8) (:) ]   column-level p-aggregated evaporation flux from soil (mm H2O/s) [+ to atm]
    qflx_evap_snow    => elm_interface_data%th%qflx_evap_snow             , &! [real(r8) (:) ]   column-level p-aggregated evaporation (inc. subl.) flux from snow (mm H2O/s) [+ to atm]
    qflx_rootsoil     => elm_interface_data%th%qflx_rootsoil              , &! [real(r8) (:,:) ] column-level p-aggregated vertically-resolved vegetation/soil water exchange (m H2O/s) (+ = to atm)
    h2osoi_liq        => elm_interface_data%th%h2osoi_liq                 , &! [real(r8) (:,:) ] liquid water (kg/m2)
    h2osoi_ice        => elm_interface_data%th%h2osoi_ice                 , &! [real(r8) (:,:) ] ice lens (kg/m2)
    qflx_et_reduced   => elm_interface_data%th%qflx_et_reduced              &! [real(r8) (:) ]   (OUTPUT) column-level p-aggregated evaporation flux from soil (mm H2O/s) [+ to atm]
    )
...
!----------------------------------------------------------------------------
    ...
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    call VecGetArrayF90(elm_pf_idata%soilliq_elms, soilliq_elms_loc, ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    call VecGetArrayF90(elm_pf_idata%soilice_elms, soilice_elms_loc, ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    call VecGetArrayF90(elm_pf_idata%effporosity_elms, effporosity_elms_loc, ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    call VecGetArrayF90(elm_pf_idata%sr_pcwmax_elms, sr_pcwmax_elms_loc, ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)

    ! for data passing from ELM --> PF
      ...
      if (mapped_gcount_skip(gcount+1)) cycle  ! skip inactive grid, but not numbering
       ...
       do j = 1, elm_pf_idata%nzelm_mapped
         ...
         if (j<=nlevgrnd) then
          ...
          soilliq  = soilliq_elms_loc(cellcount)*dz(c,j)               ! kgH2O/m2
          porosity = effporosity_elms_loc(cellcount)
          ...
          sr = 1.01_r8*sr_pcwmax_elms_loc(cellcount)*porosity         ! '1.01' will give 1% for hard-accessible pore and holding error in the calculation
          ...
          if(abs(qflx-(qflx_sink+qflx_source))>1.e-20_r8) then
              qflx_et_reduced(c) = qflx_et_reduced(c) - &                              ! '-' reverse back to original in 'qflx_rootsoil'
                                   (qflx-(qflx_sink+qflx_source)*dz(c,j)/MM2KG_M2)     ! in mmH2O/sec for ELM
          endif


          qflw_elmp_loc(cellcount) = (qflx_sink+qflx_source)                           ! PF source/sink unit: kgH2O/m3/sec
          ...
          if (j .eq. 1) then
             ...
             dsoilliq3 = min(0._r8, dsoilliq2 - qflw_elmp_loc(cellcount)*dz(c,j))
             ...
             qfluxev_top_elmp_loc(gcount+1) = -qflx_evap*MM2KG_M2       ! mmH2O/sec ==> kgH2O/m2/sec, - = out of soil
             ...
             qfluxw_top_elmp_loc(gcount+1)  = qflx_ground*MM2KG_M2     ! mmH2O/sec ==> kgH2O/m2/sec, + = potentially infiltrating into soil
          ...
          if (j .eq. elm_pf_idata%nzelm_mapped) then
             ...
             if(soilliq>0._r8) then
                ...
                qfluxw_base_elmp_loc(gcount+1) = max(dsoilliq3, -kbot*MM2KG_M2)   ! kgH2O/m2/sec
             endif
       ...
       ! NOTE: when snow-layers exist ('snl'<0), both surface energy/hydrology would be carried out separately for 'snow-column' in ELM
       ...
       if(snl(c)>=0) then
         if(qflx_et_reduced(c)>0._r8) then
           ! snow depth/water NOT enough to form a separate column in ELM snow surface processes
    ...
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    call VecRestoreArrayF90(elm_pf_idata%soilliq_elms, soilliq_elms_loc, ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    call VecRestoreArrayF90(elm_pf_idata%soilice_elms, soilice_elms_loc, ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    call VecRestoreArrayF90(elm_pf_idata%sr_pcwmax_elms, sr_pcwmax_elms_loc, ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    call VecRestoreArrayF90(elm_pf_idata%effporosity_elms, effporosity_elms_loc, ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
  ...
  !-----------------------------------------------------------------------
    ...
    snl               => elm_interface_data%snl                  , & ! number of snow layers (negative)
    !
    frac_sno_eff      => elm_interface_data%th%frac_sno_eff      , &! fraction of ground covered by snow (0 to 1)
    frac_h2osfc       => elm_interface_data%th%frac_h2osfc       , &! fraction of ground covered by surface water (0 to 1)
    !
    htvp              => elm_interface_data%th%htvp              , &! latent heat of vapor of water (or sublimation) [j/kg]
    eflx_fgr0_snow    => elm_interface_data%th%eflx_fgr0_snow    , &! heat flux from snow column (W/m**2) [+ = into soil]
    eflx_fgr0_h2osfc  => elm_interface_data%th%eflx_fgr0_h2osfc  , &! heat flux from surface water column (W/m**2) [+ = into soil]
    eflx_fgr0_soil    => elm_interface_data%th%eflx_fgr0_soil    , &! heat flux from near-surface air (W/m**2) [+ = into soil]
    eflx_rnet_soil    => elm_interface_data%th%eflx_rnet_soil    , &! heat flux between soil layer 1 and above-air, excluding SH and LE (i.e. radiation form) (W/m2) [+ = into soil]
    eflx_bot          => elm_interface_data%th%eflx_bot          , &! heat flux from beneath column (W/m**2) [+ = upward]
    t_soisno          => elm_interface_data%th%t_soisno          , &! snow-soil layered temperature [K]
    t_h2osfc          => elm_interface_data%th%t_h2osfc          , &! surface-water temperature [K]
    t_nearsurf        => elm_interface_data%th%t_nearsurf        , &! mixed air/veg. temperature near surface (for coupling with PFLOTRAN as BC)
    qflx_evap_soil    => elm_interface_data%th%qflx_evap_soil      &! non-urban column-level p-aggregated evaporation flux from soil (mm H2O/s) [+ to atm]
    )
...
!   PetscScalar, pointer :: decomp_ppools_vr_elm_loc(:)      ! (gN/m3) vertically-resolved decomposing (litter, cwd, soil) P pools
    ...
    associate ( &
      ...
      decomp_cpools_vr => elm_interface_data%bgc%decomp_cpools_vr    , & ! (gC/m3) vertically-resolved decomposing (litter, cwd, soil) c pools
      decomp_npools_vr => elm_interface_data%bgc%decomp_npools_vr    , & ! (gN/m3)  vertically-resolved decomposing (litter, cwd, soil) N pools
      smin_no3_vr      => elm_interface_data%bgc%smin_no3_vr         , & ! (gN/m3) vertically-resolved soil mineral NO3
      smin_nh4_vr      => elm_interface_data%bgc%smin_nh4_vr         , & ! (gN/m3) vertically-resolved soil mineral NH4
      smin_nh4sorb_vr  => elm_interface_data%bgc%smin_nh4sorb_vr     , & ! (gN/m3) vertically-resolved soil mineral NH4 absorbed

      decomp_ppools_vr => elm_interface_data%bgc%decomp_ppools_vr    , & ! [real(r8) (:,:,:) ! col (gP/m3) vertically-resolved decomposing (litter, cwd, soil) P pools
      solutionp_vr     => elm_interface_data%bgc%solutionp_vr        , & ! [real(r8) (:,:)   ! col (gP/m3) vertically-resolved soil solution P
      labilep_vr       => elm_interface_data%bgc%labilep_vr          , & ! [real(r8) (:,:)   ! col (gP/m3) vertically-resolved soil labile mineral P
      secondp_vr       => elm_interface_data%bgc%secondp_vr          , & ! [real(r8) (:,:)   ! col (gP/m3) vertically-resolved soil secondary mineralP
      occlp_vr         => elm_interface_data%bgc%occlp_vr            , & ! [real(r8) (:,:)   ! col (gP/m3) vertically-resolved soil occluded mineral P
      primp_vr         => elm_interface_data%bgc%primp_vr            , & ! [real(r8) (:,:)   ! col (gP/m3) vertically-resolved soil primary mineral P
      sminp_vr         => elm_interface_data%bgc%sminp_vr              & ! [real(r8) (:,:)   ! col (gP/m3) vertically-resolved soil mineral P = solutionp + labilep + secondp
    )
...
!     PetscScalar, pointer :: rate_decomp_p_elm_loc(:)     !
    ...
    associate ( &
      ...
      decomp_cpools_vr                  => elm_interface_data%bgc%decomp_cpools_vr            , &      ! (gC/m3) vertically-resolved decomposing (litter, cwd, soil) c pools
      decomp_npools_vr                  => elm_interface_data%bgc%decomp_npools_vr            , &      ! (gN/m3) vertically-resolved decomposing (litter, cwd, soil) N pools
      decomp_k_scalar_vr                => elm_interface_data%bgc%sitefactor_kd_vr            , &      ! (-) vertically-resolved decomposing rate adjusting factor relevant to location (site)
      smin_no3_vr                       => elm_interface_data%bgc%smin_no3_vr                 , &      ! (gN/m3) vertically-resolved soil mineral NO3
      smin_nh4_vr                       => elm_interface_data%bgc%smin_nh4_vr                 , &      ! (gN/m3) vertically-resolved soil mineral NH4
      smin_nh4sorb_vr                   => elm_interface_data%bgc%smin_nh4sorb_vr             , &      ! (gN/m3) vertically-resolved soil mineral NH4 absorbed
      ! plant litering and removal + SOM/LIT vertical transport
      net_to_decomp_cpools_vr           => elm_interface_data%bgc%externalc_to_decomp_cpools  , &
      net_to_decomp_npools_vr           => elm_interface_data%bgc%externaln_to_decomp_npools  , &
      ! inorg. nitrogen sink potential
      plant_ndemand_vr                  => elm_interface_data%bgc%plant_ndemand_vr            , &
      ! inorg. N source/sink
      externaln_to_nh4_vr               => elm_interface_data%bgc%externaln_to_nh4            , &
      externaln_to_no3_vr               => elm_interface_data%bgc%externaln_to_no3            , &

      net_to_decomp_ppools_vr           => elm_interface_data%bgc%externalp_to_decomp_ppools  , &
      externalp_to_primp_vr             => elm_interface_data%bgc%externalp_to_primp          , &
      externalp_to_labilep_vr           => elm_interface_data%bgc%externalp_to_labilep        , &
      externalp_to_solutionp            => elm_interface_data%bgc%externalp_to_solutionp      , &
      sminp_net_transport_vr            => elm_interface_data%bgc%sminp_net_transport_vr      , &
      plant_pdemand_vr                  => elm_interface_data%bgc%plant_pdemand_vr              &
    )
      ...
      do j = 1, elm_pf_idata%nzelm_mapped
          ...
          if(j <= nlevdecomp_full) then
              ...
              rate_plantndemand_elm_loc(cellcount) = &
                         plant_ndemand_vr(c,j)/elm_pf_idata%N_molecular_weight
  ...
  ! !LOCAL VARIABLES:
    ...
    PetscScalar, pointer :: sat_liq_elm_loc(:), sat_ice_elm_loc(:)     ! 0 - 1 of porosity
    PetscScalar, pointer :: soilliq_elm_loc(:), soilice_elm_loc(:)     ! kg/m^3
  ...
  !-----------------------------------------------------------------------
    associate ( &
      ...
      h2osoi_ice      => elm_interface_data%th%h2osoi_ice, & ! ice lens (kg/m2)
      h2osoi_vol      => elm_interface_data%th%h2osoi_vol, & ! volumetric soil water (0<=h2osoi_vol<=watsat) [m3/m3]
      zwt             => elm_interface_data%th%zwt       , & ! water table depth (m)
      zwt_perched     => elm_interface_data%th%zwt_perched, & ! perched water table depth (m)
      frost_table     => elm_interface_data%th%frost_table  & ! [Input]frost table depth (m)
     )
    !
    call VecGetArrayReadF90(elm_pf_idata%soillsat_elms, sat_liq_elm_loc, ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    call VecGetArrayReadF90(elm_pf_idata%soilliq_elms, soilliq_elm_loc, ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    ...
    if (pf_frzmode) then
       ...
       call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
       call VecGetArrayReadF90(elm_pf_idata%soilice_elms, soilice_elm_loc, ierr)
       call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    endif
      ...
      do j = 1, nlevgrnd
        ...
        if (j<=elm_pf_idata%nzelm_mapped) then
          ...
          if (pf_frzmode) then
             ...
             ! it's better to use ELM's 'watsat' for purpose of water-balance check
             porosity = watsat(c,j)
          else
             porosity = effporo_elm_loc(cellcount)
          endif

          if (pf_frzmode) then
             !isattmp = sat_ice_elm_loc(cellcount)
             isattmp = soilice_elm_loc(cellcount)/(denice*watsat(c,j))
          else
             isattmp = h2osoi_ice(c,j)/(watsat(c,j)*dz(c,j)*denice)
          endif

          if(HEAD_BASED) then
             psitmp = soilpsi_elm_loc(cellcount)                                ! -Pa
          ...
          !
             ...
             !sattmp = sat_liq_elm_loc(cellcount)
             sattmp = soilliq_elm_loc(cellcount)/(denh2o*porosity)
             psitmp = (-sucsat(c,j)*MM2PA) * ((sattmp+isattmp)**(-bsw(c,j)))    ! -Pa
             sucmin_pa = sucmin(c,j)*MM2PA                                      ! -mmH2O --> -Pa
             psitmp = min(max(psitmp,sucmin_pa),0._r8)                          ! -Pa
             h2osoi_liq(c,j) = soilliq_elm_loc(cellcount)*dz(c,j)               ! 'porosity' may be effective porosity, if 'frzmode' off
        ...
        ! just in case
          ...
          h2osoi_liq(c,j) = h2osoi_liq(c,elm_pf_idata%nzelm_mapped)/ &
                            dz(c,elm_pf_idata%nzelm_mapped)*dz(c,j)
          if (pf_frzmode) then
             h2osoi_ice(c,j) = h2osoi_ice(c,elm_pf_idata%nzelm_mapped)/ &
                            dz(c,elm_pf_idata%nzelm_mapped)*dz(c,j)
          end if
    ...
    call VecRestoreArrayReadF90(elm_pf_idata%soillsat_elms, sat_liq_elm_loc, ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    call VecRestoreArrayReadF90(elm_pf_idata%soilliq_elms, soilliq_elm_loc, ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    ...
    if (pf_frzmode) then
       ...
       call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
       call VecRestoreArrayReadF90(elm_pf_idata%soilice_elms, soilice_elm_loc, ierr)
       call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    endif
  ...
  !-----------------------------------------------------------------------
    associate ( &
       cgridcell       => col_pp%gridcell                     , & ! column's gridcell
       !
       z               => elm_interface_data%z                , & ! [real(r8) (:,:) ] layer depth (m)
       t_soisno        => elm_interface_data%th%t_soisno      , &  ! [real(r8)(:,:)] snow-soil temperature (Kelvin) [:, 1:nlevgrnd]
       frost_table     => elm_interface_data%th%frost_table     &  ! [real(r8)(:)] frost table depth (m)
       )
  ...
  ! !LOCAL VARIABLES:
    ...
    ! for PF --> ELM
    PetscScalar, pointer :: qevap_subsurf_elm_loc(:)        ! kgH2O/time-step
    ...
    ! for ELM (mpi) -->PF (Here used as previously inputs (potentials) from ELM, so that some calculation of actual quantities can be done)
    !PetscScalar, pointer :: qflw_elmp_loc(:)               !   source/sink term for plant Transpiration: unit in mass rate (kgH2O/m3/sec)
    PetscScalar, pointer :: qfluxw_top_elmp_loc(:)         !   BC in neumann flux type: unit in kgH2O/m2/sec (liq.)
    PetscScalar, pointer :: qfluxev_top_elmp_loc(:)        !   BC in neumann flux type: unit in kgH2O/m2/sec (evaporation)
    PetscScalar, pointer :: qfluxw_base_elmp_loc(:)        !   BC in neumann flux type: unit in kgH2O/m2/sec (liq.)
    PetscScalar, pointer :: press_maxponding_elmp_loc(:)   !
    ...
    frac_h2osfc       =>    elm_interface_data%th%frac_h2osfc       , & ! fraction of ground covered by surface water (0 to 1)
    !
    forc_pbot         =>    elm_interface_data%th%forc_pbot         , & ! [real(r8) (:)] atmospheric pressure (Pa)
    t_grnd            =>    elm_interface_data%th%t_grnd            , & ! [real(r8) (:)] ground surface temperature [K]
    qflx_surf         =>    elm_interface_data%th%qflx_surf         , & ! [real(r8) (:)] surface runoff (mm H2O /s)
    qflx_infl         =>    elm_interface_data%th%qflx_infl         , & ! [real(r8) (:)] soil infiltration (mm H2O /s)
    qflx_drain        =>    elm_interface_data%th%qflx_drain        , & ! [real(r8) (:,:)]  sub-surface runoff (drainage) (mm H2O /s)
    qflx_drain_vr     =>    elm_interface_data%th%qflx_drain_vr     , & ! [real(r8) (:,:)]  vertically-resolved sub-surface runoff (drainage) (mm H2O /s)
    qcharge           =>    elm_interface_data%th%qcharge             & ! [real(r8) (:)]  aquifer charge rate (mm H2O /s)
    )

    dtime = get_step_size()
    nstep = get_nstep()

    ! the previously saved in the interface data from ELM
    call VecGetArrayReadF90(elm_pf_idata%qfluxw_subsurf_elmp, qfluxw_top_elmp_loc, ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    call VecGetArrayReadF90(elm_pf_idata%qfluxev_subsurf_elmp, qfluxev_top_elmp_loc, ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    call VecGetArrayReadF90(elm_pf_idata%qfluxw_subbase_elmp, qfluxw_base_elmp_loc, ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)


    ! from PF==>ELM
    call VecGetArrayReadF90(elm_pf_idata%qevap_subsurf_elms,qevap_subsurf_elm_loc,ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
    ...
    do fc = 1,filters(ifilter)%num_soilc
      ...
      ! the following was actually from 'get_elm_bcwflx' to calculate total water potential In/Out from top
      ! in order to get potential infiltration from ELM
      qflx_infl_potential = (qfluxw_top_elmp_loc(gcount+1) +          &
                             qfluxev_top_elmp_loc(gcount+1))/MM2KG_M2   ! kgH2O/m2/sec ==> mmH2O/sec (note: ev_top already in negative for out of soil)
      qflx_infl_potential = max(0._r8, qflx_infl_potential)

      !'from PF: qevap_subsurf_elm_loc: positive - in, negative - out
      qflx_evap = qevap_subsurf_elm_loc(gcount+1)/MM2KG_M2         &
                       /dtime                                   ! kgH2O/m2/time-step ==> mmH2O/sec

      !'from PF: qinfl_subsurf_elm_loc: positive - in, negative - out
      qflx_infl(c) = qinfl_subsurf_elm_loc(gcount+1)/MM2KG_M2         &
      ...
      qflx_drain(c) = -qflux_subbase_elm_loc(gcount+1)/MM2KG_M2  &
                       *dz(c,elm_pf_idata%nzelm_mapped)/dtime          ! kgH2O/time-step ==> mmH2O/sec (+ drainage, - upward-in)
      !
    ...
    call VecRestoreArrayReadF90(elm_pf_idata%qevap_subsurf_elms,qevap_subsurf_elm_loc,ierr)
    call elm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
...
!------------------------------------------------------------------------------------
     ...
     !
     initial_cn_ratio             => elm_interface_data%bgc%initial_cn_ratio             , &
     decomp_cpools_vr             => elm_interface_data%bgc%decomp_cpools_vr         , &
     decomp_npools_vr             => elm_interface_data%bgc%decomp_npools_vr         , &
     sminn_vr                     => elm_interface_data%bgc%sminn_vr                 , &
     smin_no3_vr                  => elm_interface_data%bgc%smin_no3_vr              , &
     smin_nh4_vr                  => elm_interface_data%bgc%smin_nh4_vr              , &
     smin_nh4sorb_vr              => elm_interface_data%bgc%smin_nh4sorb_vr          , &

     decomp_cpools_delta_vr       => elm_interface_data%bgc%decomp_cpools_sourcesink  , &
     decomp_npools_delta_vr       => elm_interface_data%bgc%decomp_npools_sourcesink  , &

     sminn_to_plant_vr            => elm_interface_data%bgc%sminn_to_plant_vr         , &
     smin_no3_to_plant_vr         => elm_interface_data%bgc%smin_no3_to_plant_vr      , &
     smin_nh4_to_plant_vr         => elm_interface_data%bgc%smin_nh4_to_plant_vr      , &
     potential_immob_vr           => elm_interface_data%bgc%potential_immob_vr        , &
     actual_immob_vr              => elm_interface_data%bgc%actual_immob_vr           , &
     gross_nmin_vr                => elm_interface_data%bgc%gross_nmin_vr               &
     )
! ------------------------------------------------------------------------
     dtime = get_step_size()
          ...
          if(j <= elm_pf_idata%nzelm_mapped) then
              ...
              do k=1, ndecomp_pools
                 ...
                 decomp_cpools_delta_vr(c,j,k) = ( decomp_cpools_delta_vr(c,j,k)  &
                                + decomp_cpools_vr_elm_loc(vec_offset+cellcount)  &
                                * elm_pf_idata%C_molecular_weight ) !decomp_cpools_delta_vr=> elm_bgc_data%decomp_cpools_sourcesink
              ...
              ! currently may not be available in ELM, then add into 'smin_nh4_vr' so that avoid balance-checking issue
...
!------------------------------------------------------------------------------------
     ...
     frac_sno_eff                 => elm_interface_data%th%frac_sno_eff            , & ! fraction of ground covered by snow (0 to 1)
     frac_h2osfc                  => elm_interface_data%th%frac_h2osfc             , & ! fraction of ground covered by surface water (0 to 1)
     forc_pbot                    => elm_interface_data%th%forc_pbot               , & ! atmospheric pressure (Pa)
     !
     forc_pco2                    => elm_interface_data%bgc%forc_pco2              , & ! partial pressure co2 (Pa)
     hr_vr                        => elm_interface_data%bgc%hr_vr                  , &
     f_co2_soil_vr                => elm_interface_data%bgc%f_co2_soil_vr          , &
     f_n2o_soil_vr                => elm_interface_data%bgc%f_n2o_soil_vr          , &
     f_n2_soil_vr                 => elm_interface_data%bgc%f_n2_soil_vr           , &
     f_ngas_decomp_vr             => elm_interface_data%bgc%f_ngas_decomp_vr       , &
     f_ngas_nitri_vr              => elm_interface_data%bgc%f_ngas_nitri_vr        , &
     f_ngas_denit_vr              => elm_interface_data%bgc%f_ngas_denit_vr          &
     )
! ------------------------------------------------------------------------
     dtime = get_step_size()
     nstep = get_nstep()
...
!------------------------------------------------------------------------------------
    associate ( &
     ...
     dz                           => col_pp%dz                                           , & ! soil layer thickness depth (m)
     !
     no3_net_transport_vr         => elm_interface_data%bgc%no3_net_transport_vr  , & ! output: [c,j] (gN/m3/s)
     nh4_net_transport_vr         => elm_interface_data%bgc%nh4_net_transport_vr    & ! output: [c,j] (gN/m3/s)
     )
! ------------------------------------------------------------------------
     dtime = get_step_size()
...
!--------------------------------------------------------------------------------------
  subroutine elm_pf_BeginCBalance(elm_interface_data, bounds, filters, ifilter)
    ...
    associate(                                                              &
         decomp_cpools_vr       => elm_interface_data%bgc%decomp_cpools_vr      , &
         soil_begcb             => elm_interface_data%bgc%soil_begcb              & ! Output: [real(r8) (:)]  carbon mass, beginning of time step (gC/m**2)
         )
...
!--------------------------------------------------------------------------------------
  ...
  subroutine elm_pf_BeginNBalance(elm_interface_data, bounds, filters, ifilter)
    ...
    associate(                                                              &
         ...
         smin_nh4_vr            => elm_interface_data%bgc%smin_nh4_vr           , &
         smin_nh4sorb_vr        => elm_interface_data%bgc%smin_nh4sorb_vr       , &
         soil_begnb             => elm_interface_data%bgc%soil_begnb            , & ! Output: [real(r8) (:)]  carbon mass, beginning of time step (gC/m**2)
         soil_begnb_org         => elm_interface_data%bgc%soil_begnb_org        , & !
         soil_begnb_min         => elm_interface_data%bgc%soil_begnb_min          & !
         )
    ! calculate beginning column-level soil carbon balance, for mass conservation check
...
!--------------------------------------------------------------------------------------
  ...
  subroutine elm_pf_CBalanceCheck(elm_interface_data,bounds, filters, ifilter)
    ...
    associate(                                                                    &
         externalc               => elm_interface_data%bgc%externalc_to_decomp_cpools , & ! Input:  [real(r8) (:) ]  (gC/m2)   total column carbon, incl veg and cpool
         decomp_cpools_delta_vr  => elm_interface_data%bgc%decomp_cpools_sourcesink   , &
         hr_vr                   => elm_interface_data%bgc%hr_vr                      , &
         soil_begcb              => elm_interface_data%bgc%soil_begcb                   & ! Output: [real(r8) (:) ]  carbon mass, beginning of time step (gC/m**2)
         )
...
!--------------------------------------------------------------------------------------
  ...
  subroutine elm_pf_NBalanceCheck(elm_interface_data,bounds, filters, ifilter)
    ...
    associate(                                                                        &
         externaln_to_decomp_npools   => elm_interface_data%bgc%externaln_to_decomp_npools, & ! Input:  [real(r8) (:) ]  (gC/m2)   total column carbon, incl veg and cpool
         externaln_to_no3_vr          => elm_interface_data%bgc%externaln_to_no3          , &
         externaln_to_nh4_vr          => elm_interface_data%bgc%externaln_to_nh4          , &
         decomp_npools_delta_vr       => elm_interface_data%bgc%decomp_npools_sourcesink  , &
         decomp_npools_vr             => elm_interface_data%bgc%decomp_npools_vr          , &
