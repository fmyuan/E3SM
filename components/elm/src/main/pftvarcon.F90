module pftvarcon
  ...
  real(r8), allocatable :: woody(:)       !woody lifeform flag (0 or 1 for tree, or 2 for shrub)
  ...
  ! These arrays give information about the merge of unused crop types to the types ELM
  ! knows about. mergetoelmpft(m) gives the crop type that ELM uses to simulate input
  ! type m (and mergetoelmpft(m) == m implies that ELM simulates crop type m
  ! directly). is_pft_known_to_model(m) is true if ELM simulates crop type m, and false
  ...
  ! NOTE: using 'mergetoelmpft' variable in nc parameter file as a flag
...
!-----------------------------------------------------------------------
  ...
  !----------------------F.-M. Yuan (2018-03-23): user-defined parameter file ---------------------------------------------------------------------
    ...
    if ( .not. readv ) call ncd_io('mergetoclmpft', mergetoelmpft(0:npft-1), 'read', ncid, readvar=readv) ! this will make sure either 'mergetoelmpft' or 'mergetoclmpft' in paramfile .nc file works
    if ( .not. readv ) then
       do i = 0, mxpft
          mergetoelmpft(i) = i
       end do
  ...
  subroutine set_is_pft_known_to_model()
    ...
    ! so we can't handle it in the general loop below. But ELM always uses type 0, so
    ...
    !----------------------F.-M. Yuan: 2018-03-23---------------------------------------------------------------------
    ! 'mergetoclmpft' or 'mergetoelmpft' in the 'paramfile' is used as an indicator to user-defined parameter file
