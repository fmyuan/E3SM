...
#ifdef TPROF
            call t_startf("cplbypass_metdata_tindexing")
#endif
...
! #ifdef LDOMAIN_SUB
  ...
  subroutine lnd_import( bounds, x2l, atm2lnd_vars, glc2lnd_vars, lnd2atm_vars)
    ...
    !---------------------------------------------------------------------------
    ! !DESCRIPTION:
    ! Convert the input data from the coupler to the land model, FOR High resolution land model
    !
    ! NOTE: (1) for important very large datasets (e.g. 1 km resolution)
    !       (2) It's a version of simplified CPL_BYPASS with metdata_type of 'gswp3' or 'metsource=4'
    !       (3) assuming ldomain/surfdata and atm forcing data are ALL at same spatial resolution, AND all in same order by gridcells
    !           So that cpl_bypass format data reading doesn't need zone_mappings.txt and spatial location lookup.
    !
    ! !USES:
    use elm_varctl       , only: co2_type, co2_ppmv, iulog, use_c13, create_glacier_mec_landunit, &
                                 metdata_type, metdata_bypass, metdata_biases, co2_file, aero_file, use_atm_downscaling_to_topunit
    ...
    integer ::  ierr, varid, dimid, yr, mon, day, tod, nindex(2), caldaym(13)
    integer ::  ncid, met_ncids(14), thisncid, ng, tm
    integer ::  aindex(2), tindex(14,2), starti(3), counti(3)
    integer ::  met_nvars, nyears_spinup, nyears_trans, starti_site, endi_site
    real(r8) :: smap05_lat(360), smap05_lon(720)
    real(r8) :: smapt62_lat(94), smapt62_lon(192)
    ...
    if (atm2lnd_vars%loaded_bypassdata == 0) then
          ...
          if (index(metdata_type,'gswp3') .gt. 0 .and. index(metdata_type, 'daymet') .gt. 0) then
            atm2lnd_vars%metsource = 4
            use_daymet = .true.
          else
            call endrun( sub//' ERROR: Invalid met data source for cpl_bypass - "gswp3_daymet", rather than: ' // metdata_type )
          end if
          ...
          do v=1,met_nvars
            ...
            counti(1) = 2
            ierr = nf90_inq_varid(met_ncids(v), 'DTIME', varid)
            ierr = nf90_get_var(met_ncids(v), varid, temp_t, starti(1:1), counti(1:1))
            atm2lnd_vars%timeres(v)        = (temp_t(2)-temp_t(1))*24._r8
            atm2lnd_vars%npf(v)            = 86400d0*(temp_t(2)-temp_t(1))/get_step_size()
            atm2lnd_vars%timelen_spinup(v) = nyears_spinup*(365*nint(24./atm2lnd_vars%timeres(v)))

            ! grid numbers (assumed exactly same for all met variables)
            ierr = nf90_inq_dimid(met_ncids(v), 'n', dimid)
            ierr = nf90_Inquire_Dimension(met_ncids(v), dimid, len = atm2lnd_vars%atm_ngrid)
    ...
    endif ! end-if (atm2lnd_vars%loaded_bypassdata == 0)

#endif
! read-in CPL_BYPASS datasets, if any
    call mpi_barrier(mpicom, ierr)

    !-----------------------------------------------------------------------------------------------------!
    ! data unpacking/interpolating, if any
    ...
    do g = bounds%begg,bounds%endg
       ...
       !
        ...
        !--------------------------------------------------------------------------------------------------------------
        ! scaling or offsetting variables
...
#endif
       ! done with data interpolating (cpl_bypass) or assigning from atm
