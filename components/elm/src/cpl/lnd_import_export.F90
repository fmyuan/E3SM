...
10          continue
  ...
  !---------------------------------- CO2 -------------------------------------------------------------------
       ...
       if (implicit_stress) then
          atm2lnd_vars%forc_wsresp_grc(g)  = 0._r8                                       !           Atm state m/s/Pa
          atm2lnd_vars%forc_tau_est_grc(g) = 0._r8                                       !           Atm state Pa
       end if
       atm2lnd_vars%forc_ugust_grc(g) = 0._r8                                            !           Atm state m/s
  ...
  !-----------------------------------------------------------------------------------------------------
       ...
       !
       if (implicit_stress) then
          atm2lnd_vars%forc_wsresp_grc(g)  = x2l(index_x2l_Sa_wsresp,i) !           Atm state m/s/Pa
          atm2lnd_vars%forc_tau_est_grc(g) = x2l(index_x2l_Sa_tau_est,i)!           Atm state Pa
       end if
       if (atm_gustiness) then
          atm2lnd_vars%forc_ugust_grc(g)   = x2l(index_x2l_Sa_ugust,i)  !           Atm state m/s
       else
          atm2lnd_vars%forc_ugust_grc(g)   = 0._r8
       end if
       ...
       do topo = grc_pp%topi(g), grc_pp%topf(g)
         ...
         top_as%wsresp(topo)  = atm2lnd_vars%forc_wsresp_grc(g)                !           Atm state m/s/Pa
         top_as%tau_est(topo) = atm2lnd_vars%forc_tau_est_grc(g)               !           Atm state Pa
         top_as%ugust(topo)   = atm2lnd_vars%forc_ugust_grc(g)                 !           Atm state m/s
