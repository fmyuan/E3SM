...
#include "shr_assert.h"
  ...
  type(dyn_var_time_uninterp_type) :: soilamend_specpct   ! soil amendment species percentage [-] (yr, species, lat, lon)
...
contains
  !-----------------------------------------------------------------------
  subroutine dynsoilamendments_init(bounds, dynsoilamendments_filename, namendspec)
    ...
    soilamend_specpct = dyn_var_time_uninterp_type( &
         ...
         do_check_sums_equal_1=.true., data_shape=amendspec_shape, &
         allow_nodata=.true.)

  end subroutine dynsoilamendments_init

  !-----------------------------------------------------------------------
  subroutine dynsoilamendments_appl(bounds, appl_rate, appl_grainsize, appl_specpct)
    ...
    ! !ARGUMENTS:
    type(bounds_type), intent(in) :: bounds  ! proc-level bounds
    real(r8), pointer, intent(out):: appl_rate(:)      ! col-level
    real(r8), pointer, intent(out):: appl_grainsize(:) ! col-level
    real(r8), pointer, intent(out):: appl_specpct(:,:) ! col-level
    ...
    ! not sure if here data obtained is averged for every day/time or instant (TODO: checking)
    ...
    do p = bounds%begp, bounds%endp
       ...
       if (lun_pp%itype(l) == istcrop .or. lun_pp%itype(l) == istsoil) then
          ...
          ! assuming application occurs at mid-night of the day of year
          ! this wouldn't make large impact, but if it is then needs more thoughts on when to add into column.
          if (appl_doy_cur(g,ti,m) == nextsw_cday_mod .and. &
              secs_curr == 0._r8) then
