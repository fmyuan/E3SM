      ...
      do f = 1, fn
         ...
         if (use_finetop_rad) then
            ...
            dlrad(p) = (1._r8-emv(p))*emg(c)*forc_lwrad(t) + &
                  emv(p)*emg(c)*sb*tlbef(p)**3._r8*(tlbef(p) + 4._r8*dt_veg(p))/cos(slope_rad)
         else
            dlrad(p) = (1._r8-emv(p))*emg(c)*forc_lwrad(t) + &
                  emv(p)*emg(c)*sb*tlbef(p)**3._r8*(tlbef(p) + 4._r8*dt_veg(p))
         endif
         ! Upward longwave radiation above the canopy
         if (use_finetop_rad) then
            slope_rad = slope_deg(g) * deg2rad
            ulrad(p) = ((1._r8-emg(c))*(1._r8-emv(p))*(1._r8-emv(p))*forc_lwrad(t) &
                + emv(p)*(1._r8+(1._r8-emg(c))*(1._r8-emv(p)))*sb*tlbef(p)**3._r8*(tlbef(p) + &
