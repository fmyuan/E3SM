...
#include "shr_assert.h"
  ...
  use pftvarcon           , only : nonvascular
...
contains
    ...
    associate(                                                       &
      ...
      do f = 1, fn
        ...
        do iv = 1, nrad(p)
            ...
            if (par_z(p,iv) <= 0._r8) then           ! night time
            !if (nonvascular(veg_pp%itype(p)) == 1) lmr_z(p,iv) = lmr_z(p,iv) * wcscaler
...
#if (defined HUM_HOL)
               if (nonvascular(veg_pp%itype(p)) == 1) gs_mol(p,iv) = bbb(p)
#endif
  ...
  subroutine ci_func(ci, fval, p, iv, c, t, gb_mol, je, cair, oair, lmr_z, par_z,&
    ...
    associate(&
      ...
      gs_mol = max(r1,r2)
#if (defined HUM_HOL)
      if (nonvascular(veg_pp%itype(p)) == 1) gs_mol = bbb(p)
#endif
      ! Derive new estimate for ci

      fval =ci - cair + an(p,iv) * forc_pbot(t) * (1.4_r8*gs_mol+1.6_r8*gb_mol) / (gb_mol*gs_mol)
#if (defined HUM_HOL)
      if (nonvascular(veg_pp%itype(p)) == 1) fval = ci - cair + an(p,iv) *forc_pbot(c)/gs_mol
#endif
    end associate
