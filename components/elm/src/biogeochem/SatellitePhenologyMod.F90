...
contains
  ...
  subroutine SatellitePhenology(bounds, num_filter, filter, &
    ...
    associate(                                                           &
         ...
         annlai             => canopystate_vars%annlai_patch  ,          & ! Input: [real(r8) (:) ]
         ...
         if (.not. use_lai_streams) then
            ...
            if (.not. use_cn .and. season_decid(ivt(p)) == 1._r8) then
               ...
               if (ws_flag == 1._r8 .and. phen_spring == 0) then 
                 ...
                 !PAR model
                 if (t_ref2m(p) > phen_tbase) then
                   sp_gdd(p) = sp_gdd(p) + (28.4_r8 / (1._r8 + exp(3.4_r8 - (t_ref2m(p) -SHR_CONST_TKFRZ)*0.185_r8)))*fracday
                 endif
